...
def State_Space_Generation(AoA_trim,U_cruise,Mass,Ix,Iy,Iz,Ixz,Long_res_dict,Lat_res_dict,Velocity_incr,AngVelocity_incr,Xw_dot,Zw_dot,Mw_dot,Trim,theta_value):
    ...
    # Longitudinal Motion
    Xu = (Long_res_dict["Wind Movements"]["u"]["Force X"][1]-Long_res_dict["Wind Movements"]["u"]["Force X"][0])/(2*Velocity_incr)
    Zu = (Long_res_dict["Wind Movements"]["u"]["Force Z"][1]-Long_res_dict["Wind Movements"]["u"]["Force Z"][0])/(2*Velocity_incr)
    Mu = (Long_res_dict["Wind Movements"]["u"]["Moment Y"][1]-Long_res_dict["Wind Movements"]["u"]["Moment Y"][0])/(2*Velocity_incr)

    Xw = (Long_res_dict["Wind Movements"]["w"]["Force X"][1]-Xw -Long_res_dict["Wind Movements"]["w"]["Force X"][0])/(2*Velocity_incr)
    Zw = (Long_res_dict["Wind Movements"]["w"]["Force Z"][1]-Long_res_dict["Wind Movements"]["w"]["Force Z"][0])/(2*Velocity_incr)
    Mw = (Long_res_dict["Wind Movements"]["w"]["Moment Y"][1]-Long_res_dict["Wind Movements"]["w"]["Moment Y"][0])/(2*Velocity_incr)

    Xq = (Long_res_dict["Body Movements"]["q"]["Force X"][1]-Long_res_dict["Body Movements"]["q"]["Force X"][0])/(2*AngVelocity_incr)
    Zq = (Long_res_dict["Body Movements"]["q"]["Force Z"][1]-Long_res_dict["Body Movements"]["q"]["Force Z"][0])/(2*AngVelocity_incr)
    Mq = (Long_res_dict["Body Movements"]["q"]["Moment Y"][1]-Long_res_dict["Body Movements"]["q"]["Moment Y"][0])/(2*AngVelocity_incr)
    ...
    Long_Mat[0,3]= -9.81*np.cos(theta)-(Xw_dot*9.81*np.sin(theta))/((Mass-Zw_dot))
    ...
    Yv = (Lat_res_dict["Wind Movements"]["v"]["Force Y"][1]-Lat_res_dict["Wind Movements"]["v"]["Force Y"][0])/(2*Velocity_incr)
    Lv = (Lat_res_dict["Wind Movements"]["v"]["Moment X"][1]-Lat_res_dict["Wind Movements"]["v"]["Moment X"][0])/(2*Velocity_incr)
    Nv = (Lat_res_dict["Wind Movements"]["v"]["Moment Z"][1]-Lat_res_dict["Wind Movements"]["v"]["Moment Z"][0])/(2*Velocity_incr)

    Yp = (Lat_res_dict["Body Movements"]["p"]["Force Y"][1]-Lat_res_dict["Body Movements"]["p"]["Force Y"][0])/(2*AngVelocity_incr)
    Lp = (Lat_res_dict["Body Movements"]["p"]["Moment X"][1]-Lat_res_dict["Body Movements"]["p"]["Moment X"][0])/(2*AngVelocity_incr)
    Np = (Lat_res_dict["Body Movements"]["p"]["Moment Z"][1]-Lat_res_dict["Body Movements"]["p"]["Moment Z"][0])/(2*AngVelocity_incr)

    Yr = (Lat_res_dict["Body Movements"]["r"]["Force Y"][1]-Lat_res_dict["Body Movements"]["r"]["Force Y"][0])/(2*AngVelocity_incr)
    Lr = (Lat_res_dict["Body Movements"]["r"]["Moment X"][1]-Lat_res_dict["Body Movements"]["r"]["Moment X"][0])/(2*AngVelocity_incr)
    Nr = (Lat_res_dict["Body Movements"]["r"]["Moment Z"][1]-Lat_res_dict["Body Movements"]["r"]["Moment Z"][0])/(2*AngVelocity_incr)
