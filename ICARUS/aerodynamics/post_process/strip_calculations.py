...
def get_potential_loads(
    ...
    for i in jnp.arange(0, plane.NM):
        M += L_pan[i] * jnp.cross(plane.control_points[i, :] - plane.CG, plane.control_nj[i, :])
        M += D_pan[i] * jnp.cross(plane.control_points[i, :] - plane.CG, plane.control_nj[i, :])
    Mx, My, Mz = M
...
def get_pseudo_viscous_loads(
    ...
    try:
        (L_2D, D_2D, My_2D, CL_2D, CD_2D, Cm_2D) = integrate_polars_from_reynolds(plane=plane, state=state)
