...
class ConceptAirplane():
    ...
    def __init__(
        ...
        self.missing_vals = self.get_missing_parameters()
    ...
    def CL_CRUISE(self):
        ...
        if self.ASPECT_RATIO is None:
            return None
        return np.sqrt(self.CD_0 * (np.pi * self.OSWALD_CRUISE * self.ASPECT_RATIO)) # / np.sqrt(1 - self.CRUISE_MACH**2)
    ...
    def set_parameters(self, missing_val_dict: dict):
        rest_of_args = {}
    ...
    def supress_parameters(self, values_to_suppress: dict):
    ...
    def far_criteria(self, **missing_vals):
    ...
    def partial_fun_factory(self,objective_function,args_to_suppress= [], **kwargs):
        not_defined = [arg for arg in args_to_suppress or self.get_missing_parameters()]
        ...
        def optimize_fun(new_args, **new_kwargs):
            ...
            if len(not_defined) > 1:
                args_to_set.update(
                    {k:v for k,v in zip(not_defined,new_args)}
                )
            elif len(not_defined) == 1:
                args_to_set.update(
                    {not_defined[0]:new_args[0]}
                )
