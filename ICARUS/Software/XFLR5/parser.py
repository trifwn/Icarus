...
from ICARUS.Vehicle.wing import Wing, define_linear_chord, define_linear_span
from ICARUS.Airfoils.airfoilD import AirfoilD

def parse_xfl_project(filename) -> Airplane:
    """
    ...
    point_masses: list[tuple[float, ndarray]] = []
    for pmass in plane['Inertia']['Point_Mass']:
        m = float(pmass['Mass'])
        coor: ndarray[Any, dtype[floating]] = np.array(
    ...
    origin: ndarray[Any, dtype[floating]] = np.array([0.0, 0.0, 0.0], dtype=floating)
