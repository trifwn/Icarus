...
def GNVPdstCase(plane, db, solver2D, maxiter, timestep,
                Uinf, angle, environment, bodies, dst, analysis,
                solver_options):
...
def runGNVPpertr(plane, db, solver2D, maxiter, timestep,
    ...
    for dst in plane.disturbances:
        msg = GNVPdstCase(plane, db, solver2D, maxiter,
                          timestep, Uinf, angles, environment,
                          bodies, dst, "Dynamics", solver_options)
        print(msg)
...
def runGNVPpertrParallel(plane,environment, db, solver2D, maxiter, timestep, Uinf, angles,
    ...
    with Pool(12) as pool:
        args_list = [(plane, db, solver2D, maxiter,
                      timestep, Uinf, angles, environment,
                      bodies, dst, "Dynamics", solver_options) for dst in disturbances]
...
def runGNVPsensitivity(plane,environment, var, db, solver2D, maxiter, timestep, Uinf, angles,
    ...
    for dst in plane.sensitivity[var]:
        msg = GNVPdstCase(plane, db, solver2D, maxiter,
                          timestep, Uinf, angles, environment,
                          bodies, dst, "Sensitivity", solver_options)
        print(msg)
...
def runGNVPsensitivityParallel(plane,environment, var, db, solver2D, maxiter, timestep, Uinf, angles,
    ...
    with Pool(12) as pool:
        args_list = [(plane, db, solver2D, maxiter,
                      timestep, Uinf, angles, environment,
                      bodies, dst, f"Sensitivity_{dst.var}", solver_options) for dst in disturbances]
