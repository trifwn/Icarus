...
class JaxAirfoil:
    ...
    def create_batch_from_list(
        ...
        """
        Create batch arrays from a list of JaxAirfoil instances.

        Args:
            airfoils: List of JaxAirfoil instances to batch
            target_buffer_size: Optional target buffer size (auto-determined if None)
        ...
        if target_buffer_size is None:
            airfoil_sizes = [airfoil.n_points for airfoil in airfoils]
            target_buffer_size = BatchAirfoilOps.determine_batch_buffer_size(
                airfoil_sizes,
            )

        batch_size = len(airfoils)

        # Initialize batch arrays
        batch_coords = jnp.full((batch_size, 2, target_buffer_size), jnp.nan)
        batch_validity_masks = jnp.zeros((batch_size, target_buffer_size), dtype=bool)
        upper_split_indices = []
        n_valid_points = []

        # Process each airfoil
        for i, airfoil in enumerate(airfoils):
            # Resize airfoil buffer if needed
            if airfoil.buffer_size < target_buffer_size:
                # Pad the airfoil's coordinates to target size
                padded_coords = AirfoilBufferManager.pad_coordinates(
                    airfoil._coordinates,
                    target_buffer_size,
                )
                ...
                # Use existing coordinates (truncate if larger)
                padded_coords = airfoil._coordinates[:, :target_buffer_size]
                validity_mask = airfoil._validity_mask[:target_buffer_size]

            # Store in batch arrays
            batch_coords = batch_coords.at[i].set(padded_coords)
            batch_validity_masks = batch_validity_masks.at[i].set(validity_mask)
            upper_split_indices.append(airfoil._upper_split_idx)
            n_valid_points.append(airfoil.n_points)

        return batch_coords, batch_validity_masks, upper_split_indices, n_valid_points

    @classmethod
    def batch_from_coordinates_list(
        cls,
        coordinates_list: List[Union[Array, np.ndarray]],
        ...
        """
        Create batch arrays from a list of coordinate arrays.

        Args:
            coordinates_list: List of coordinate arrays in selig format
            names: Optional list of names for each airfoil
            target_buffer_size: Optional target buffer size (auto-determined if None)
        ...
        if not coordinates_list:
            raise ValueError("Cannot create batch from empty list of coordinates")

        # Create individual airfoils first
        airfoils = []
        for i, coords in enumerate(coordinates_list):
            name = names[i] if names and i < len(names) else f"Airfoil_{i}"
            airfoil = cls(coords, name=name)
            airfoils.append(airfoil)

        # Use the existing batch creation method
        return cls.create_batch_from_list(airfoils, target_buffer_size)
    ...
    def batch_naca4(
        ...
        """
        Create a batch of NACA 4-digit airfoils.

        Args:
            digits_list: List of NACA 4-digit designations (e.g., ["2412", "0012"])
            n_points: Number of points for each surface
            target_buffer_size: Optional target buffer size (auto-determined if None)
        ...
        # Parse NACA parameters for batch generation
        batch_max_camber = []
        ...
        for digits in digits_list:
            if not isinstance(digits, str) or len(digits) != 4 or not digits.isdigit():
                raise ValueError(
                    f"NACA 4-digit designation must be a 4-digit string, got {digits}",
                )
            ...
            batch_max_camber.append(M)
            batch_camber_position.append(P)
            batch_thickness.append(XX)

        # Convert to JAX arrays
        batch_max_camber = jnp.array(batch_max_camber)
        batch_camber_position = jnp.array(batch_camber_position)
        batch_thickness = jnp.array(batch_thickness)

        # Generate batch coordinates
        batch_upper_coords, batch_lower_coords = (
            BatchAirfoilOps.batch_generate_naca4_coordinates(
                batch_max_camber,
                batch_camber_position,
                batch_thickness,
                n_points,
        ...
        # Convert to selig format and create batch
        batch_size = len(digits_list)

        # Determine buffer size
        if target_buffer_size is None:
            target_buffer_size = BatchAirfoilOps.determine_batch_buffer_size(
                [2 * n_points] * batch_size,
            )

        # Convert each airfoil to selig format and pad
        batch_coords = jnp.full((batch_size, 2, target_buffer_size), jnp.nan)
        batch_validity_masks = jnp.zeros((batch_size, target_buffer_size), dtype=bool)
        upper_split_indices = []
        n_valid_points = []
        ...
        for i in range(batch_size):
            # Convert to selig format
            selig_coords = CoordinateProcessor.to_selig_format(
                batch_upper_coords[i],
                batch_lower_coords[i],
            )
            ...
            batch_coords = batch_coords.at[i].set(padded_coords)
            batch_validity_masks = batch_validity_masks.at[i].set(validity_mask)
            upper_split_indices.append(n_points)  # Upper surface has n_points
            n_valid_points.append(n_valid)
    ...
    def batch_thickness(
        ...
        Args:
            batch_coords: Batch coordinate arrays (batch_size, 2, buffer_size)
            upper_split_indices: List of upper surface split indices for each airfoil
            n_valid_points: List of valid point counts for each airfoil
            query_x: X coordinates to query thickness at
            target_buffer_size: Buffer size used for the batch
        ...
        # Split into upper and lower surfaces for each airfoil
        batch_upper_coords = jnp.full((batch_size, 2, target_buffer_size), jnp.nan)
        batch_lower_coords = jnp.full((batch_size, 2, target_buffer_size), jnp.nan)

        for i in range(batch_size):
            upper_split_idx = upper_split_indices[i]
            n_valid = n_valid_points[i]

            # Extract upper and lower surfaces
            upper_coords = batch_coords[i, :, :upper_split_idx]
            lower_coords = batch_coords[i, :, upper_split_idx:n_valid]

            # Pad to target size
            upper_padded = jnp.concatenate(
                [
                    upper_coords,
                    jnp.full((2, target_buffer_size - upper_coords.shape[1]), jnp.nan),
                ],
                axis=1,
            )
            ...
            lower_padded = jnp.concatenate(
                [
                    lower_coords,
                    jnp.full((2, target_buffer_size - lower_coords.shape[1]), jnp.nan),
                ],
                axis=1,
            )

            batch_upper_coords = batch_upper_coords.at[i].set(upper_padded)
            batch_lower_coords = batch_lower_coords.at[i].set(lower_padded)

        # Use the maximum valid counts for JIT compatibility
        max_upper_valid = max(upper_split_indices)
        max_lower_valid = max(
            n_valid - upper_split
            for n_valid, upper_split in zip(n_valid_points, upper_split_indices)
        )

        return BatchAirfoilOps.batch_compute_thickness(
            batch_upper_coords,
            batch_lower_coords,
            max_upper_valid,
            max_lower_valid,
    ...
    @staticmethod
    def batch_morph(
        batch_coords1: Float[Array, "batch_size 2 buffer_size"],
        batch_coords2: Float[Array, "batch_size 2 buffer_size"],
        batch_validity_masks1: Bool[Array, "batch_size buffer_size"],
        batch_validity_masks2: Bool[Array, "batch_size buffer_size"],
        eta: Union[float, Float[Array, "batch_size"]],
    ) -> Float[Array, "batch_size 2 buffer_size"]:
        """
        Morph between pairs of airfoils in batch mode.

        Args:
            batch_coords1: First batch of airfoil coordinates (batch_size, 2, buffer_size)
            batch_coords2: Second batch of airfoil coordinates (batch_size, 2, buffer_size)
            batch_validity_masks1: Validity masks for first batch (batch_size, buffer_size)
            batch_validity_masks2: Validity masks for second batch (batch_size, buffer_size)
            eta: Morphing parameter(s) where 0=first airfoil, 1=second airfoil
                 Can be a single float (applied to all pairs) or array of floats (one per pair)
        ...
        if isinstance(eta, (float, int)):
            eta = jnp.full(batch_coords1.shape[0], eta)
        elif isinstance(eta, np.ndarray):
            eta = jnp.array(eta)

        return BatchAirfoilOps.batch_morph_airfoils(
            batch_coords1,
            batch_coords2,
            batch_validity_masks1,
            batch_validity_masks2,
            eta,
        )

    @staticmethod
    def batch_apply_flap(
        batch_coords: Float[Array, "batch_size 2 buffer_size"],
        upper_split_indices: List[int],
        n_valid_points: List[int],
        flap_hinge_chord_percentages: Union[float, List[float], Array],
        flap_angles: Union[float, List[float], Array],
        target_buffer_size: int,
        flap_hinge_thickness_percentage: float = 0.5,
        chord_extension: float = 1.0,
    ) -> Float[Array, "batch_size 2 buffer_size"]:
        """
        Apply flap transformations to a batch of airfoils.

        Args:
            batch_coords: Batch coordinate arrays (batch_size, 2, buffer_size)
            upper_split_indices: List of upper surface split indices for each airfoil
            n_valid_points: List of valid point counts for each airfoil
            flap_hinge_chord_percentages: Flap hinge positions (0.0 to 1.0)
            flap_angles: Flap deflection angles in degrees (positive = downward)
            target_buffer_size: Buffer size used for the batch
            flap_hinge_thickness_percentage: Position of hinge through thickness
        ...
        # Convert parameters to arrays if needed
        if isinstance(flap_hinge_chord_percentages, (float, int)):
            flap_hinge_chord_percentages = [flap_hinge_chord_percentages] * batch_size
        if isinstance(flap_angles, (float, int)):
            flap_angles = [flap_angles] * batch_size

        flap_hinge_chord_percentages = jnp.array(flap_hinge_chord_percentages)
        flap_angles = jnp.deg2rad(jnp.array(flap_angles))  # Convert to radians

        # Split into upper and lower surfaces for each airfoil
        batch_upper_coords = jnp.full((batch_size, 2, target_buffer_size), jnp.nan)
        batch_lower_coords = jnp.full((batch_size, 2, target_buffer_size), jnp.nan)

        # Calculate flap hinge x-coordinates for each airfoil
        batch_flap_hinge_x = jnp.zeros(batch_size)
        ...
        for i in range(batch_size):
            ...
            # Calculate flap hinge x-coordinate
            min_x = jnp.min(batch_coords[i, 0, :n_valid])
            max_x = jnp.max(batch_coords[i, 0, :n_valid])
            flap_hinge_x = min_x + flap_hinge_chord_percentages[i] * (max_x - min_x)
            batch_flap_hinge_x = batch_flap_hinge_x.at[i].set(flap_hinge_x)
        ...
        # Apply flap transformations
        new_upper_coords, new_lower_coords, _, _ = (
            BatchAirfoilOps.batch_apply_flap_transformation(
                batch_upper_coords,
                batch_lower_coords,
                max_upper_valid,
                max_lower_valid,
        ...
        # Combine upper and lower surfaces back into selig format
        batch_result = jnp.full((batch_size, 2, target_buffer_size), jnp.nan)

        for i in range(batch_size):
            upper_split_idx = upper_split_indices[i]

            # Extract valid portions
            upper_valid = new_upper_coords[i, :, :upper_split_idx]
            lower_valid = new_lower_coords[i, :, :max_lower_valid]

            # Combine into selig format
            selig_coords = jnp.concatenate([upper_valid, lower_valid], axis=1)

            # Pad and store
            if selig_coords.shape[1] < target_buffer_size:
                selig_padded = jnp.concatenate(
                    [
                        selig_coords,
                        jnp.full(
                            (2, target_buffer_size - selig_coords.shape[1]),
                            jnp.nan,
                        ),
                    ],
                    axis=1,
                ...
                selig_padded = selig_coords[:, :target_buffer_size]

            batch_result = batch_result.at[i].set(selig_padded)
    ...
    def batch_y_upper(
        ...
        # Extract upper surface coordinates for each airfoil
        batch_upper_coords = jnp.full((batch_size, 2, target_buffer_size), jnp.nan)
        ...
        # Use the maximum valid count for JIT compatibility
        max_upper_valid = max(upper_split_indices)

        return BatchAirfoilOps.batch_y_upper(
            batch_upper_coords,
            max_upper_valid,
            query_x,
        )
    ...
    def batch_y_lower(
        ...
        # Extract lower surface coordinates for each airfoil
        batch_lower_coords = jnp.full((batch_size, 2, target_buffer_size), jnp.nan)
        ...
        return BatchAirfoilOps.batch_y_lower(
            batch_lower_coords,
            max_lower_valid,
            query_x,
        )
    ...
    def batch_camber_line(
        ...
        return BatchAirfoilOps.batch_compute_camber_line(
            batch_upper_coords,
            batch_lower_coords,
            max_upper_valid,
            max_lower_valid,
